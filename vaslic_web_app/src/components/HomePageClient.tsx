...
// ─── Hero slices ─────────────────────────────────────────────────────────────
const heroSlices = [
    { label: "Gothic", font: "'UnifrakturMaguntia', cursive", italic: false, tracking: "", uppercase: false, image: "https://lh3.googleusercontent.com/aida-public/AB6AXuCUTDlsQeP6lPV-nR5LaRXmf2ANnGcalWmBaum5MwM6FXPz09zvU7q6S3TMZikwZQpcW9vcHXjexrPezbzg8NuUFoEkf30RmoJo8rrMbQH3Icmk1Zj-7b0vE-uBer9U_X7DOfuScQ535FEKGpP5MMsHdRafLSSfhc03BbeLE7JslgBhrvbXpbZhhJV-vxdVDfQe0YsIKpieZ1qWckd9tD7ZJ0DhSxapkXkjWqImkftfut3tgpmT-ZOIZNulYHtXafXTPXber_lkCQ0", href: "/gothic" },
    { label: "Bohemian", font: "'Playfair Display', serif", italic: true, tracking: "", uppercase: false, image: "https://lh3.googleusercontent.com/aida-public/AB6AXuAbnP01Ukg8pgd17yyKws2akaGZoCK3HIXcksOtjeQq928Z2n6jF_7ArvGbsBwyLmmUJpX-kNj4vUTTXlscpHsstzCu0zi9hBaJuKms2lh4BoQt3f1Z9ixZWqAXUx7qS0wNG0y3XilEMv60vKTlEbb_BzP_-1ddvHztCtz8bEivEYPFJIG9VCYfHBxqxnHCPHKm1IUZGMKoLFIEj6JkSErSsIHwIP4wHkJSNTamRBCgOx2GGJGOHDbywBqpLP45OQIGles4kW_-zjA", href: "/bohemian" },
    { label: "Avant", font: "'Bebas Neue', sans-serif", italic: false, tracking: "tracking-widest", uppercase: false, image: "https://lh3.googleusercontent.com/aida-public/AB6AXuBOpO5lF9xS3Z2l1uB5lZo4lcfLib6hdzt-bglgA638S3c3idsHkMngoWkTrCDRqQtgocppTsAxtZmvN_NlXe_Jzo-xePSn8rn4qpZB5Wam62_ol5MN6PnIxQTaZ2mbR_3XzqEy9rI8C2up2PbisSKg3Oj5u8ISzZ70zJ3mB-qt1OfH8AHgrsf_BGdwJCpabtH740T5lb_PXNQe5gmzji0x2Nmi95i0I6uLlCCBq3qwZlHL5XrEib7myefEjOhys8AMFqbHLCscaTg", href: "/avant-garde" },
    { label: "Street", font: "'Bebas Neue', sans-serif", italic: false, tracking: "", uppercase: true, image: "https://lh3.googleusercontent.com/aida-public/AB6AXuCFW2CLM5hkaF1DvZEM7D83E985ECLYkZOReOq1vTSzrn88bYVwRBxgn6GxDeyANAyBaagkvwkptXkhhib-RwJQqZJ9AT8U1408WskuElwq6wRnolzZumqpzdIpgYhtF3UCsuvt5XrmB04eYTuvF9NXkXwlIpvd-V4AhvwobgKTdkBRYFJhSTYhnerxHdtiofMglHNW5vXJGr8brdUWVodUU1L6kqZ6Zt17tVFVCg4H3d_nn23RIQYgz-oOdC3oACHQGFuZCgA6yjQ", href: "/street" },
    { label: "Funky", font: "'Righteous', sans-serif", italic: false, tracking: "", uppercase: false, image: "https://lh3.googleusercontent.com/aida-public/AB6AXuCAWllXXPFmv4PP22dWuEtn6IP5oBS-nUQ95tNbgQAC-jr2ApQqBIJJ3ljXGH0OrtPYC5cvBJ9JaCQXRGGDQRyS56JJQHCJ5tRRbC6kW4b-6OaqdGxq4gDsZ7hs-tvipv-CU7qoRyJ66-mCpYtAe_aLvLntDlY9p_8fzBA9KkkNFA5OI946BbALAKr_7oeUsEFBbJr-4i4srG2j3gyVxlBU7nwPjgfLc86V4x8Xk3n3TRGWJ3vbn6NF-1KLUkUcOfRCejvlk1E7-eE", href: "/funky" },
];
...
function ProductCard({
    p, bg, delay, slug, idColor, editionStyle, nameStyle, unitStyle, borderStyle, arrowColor,
}: {
    p: any; bg?: string; delay: number; slug?: string; idColor: string; editionStyle: React.CSSProperties; nameStyle: React.CSSProperties; unitStyle: React.CSSProperties; borderStyle: React.CSSProperties; arrowColor: string;
}) {
    const unitsRemaining = p.sizes?.reduce((acc: number, s: any) => acc + s.units_remaining, 0) || 0;
    // Let's assume initially total units was 100 for percentage visualization
    const totalUnits = 100;

    const card = (
        <div
            className="group relative aspect-[3/4] p-8 flex flex-col justify-between overflow-hidden"
            style={{ background: bg ?? "#1e1e1e", transition: "transform 0.6s cubic-bezier(0.34,1.56,0.64,1)", transitionDelay: `${delay}ms`, cursor: slug ? "pointer" : "default" }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.transform = "scale(1.02) translateY(-10px)"; }}
        ...
        >
            ...
            <div className="relative z-10 flex justify-between items-start">
                <span className="text-[10px] tracking-widest" style={{ fontFamily: "'Space Grotesk', sans-serif", color: idColor }}>{p.vault_id}</span>
                <span className="px-2 py-1 text-[9px] font-bold" style={editionStyle}>{p.status.replace('_', ' ').toUpperCase()}</span>
            </div>
            <div className="relative z-10">
                <h4 className="font-bold mb-4" style={nameStyle}>{p.name}</h4>
                <div className="pt-4" style={{ borderTop: "1px solid", ...borderStyle }}>
                    <UnitsCounter productId={p.id} initial={unitsRemaining} total={totalUnits} />
                </div>
            </div>
        </div>
    );
...
export function HomePageClient({ products }: { products: any[] }) {
    const [email, setEmail] = useState("");

    const gothicProducts = products.filter(p => p.category?.slug === 'gothic' && p.status !== 'retired');
    const bohemianProducts = products.filter(p => p.category?.slug === 'bohemian' && p.status !== 'retired');
    const avantProducts = products.filter(p => p.category?.slug === 'avant-garde' && p.status !== 'retired');
    const streetProducts = products.filter(p => p.category?.slug === 'street' && p.status !== 'retired');
    const funkyProducts = products.filter(p => p.category?.slug === 'funky' && p.status !== 'retired');

    const retiredDesigns = products.filter(p => p.status === 'retired');
            ...
            <section className="relative h-screen w-full flex overflow-hidden bg-black">
                {heroSlices.map((s) => (
                    <Link
                        ...
                        onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.flex = "3"; }}
                        onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.flex = "1"; }}
                    >
                        <img src={s.image} alt={s.label} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-[2000ms]" style={{ opacity: 0.5 }} />
                        <span
                            ...
                            style={{ fontFamily: s.font, fontStyle: s.italic ? "italic" : undefined, fontWeight: s.italic ? 700 : undefined, textTransform: s.uppercase ? "uppercase" : undefined, letterSpacing: s.tracking ? "0.1em" : undefined }}
                        >
                            {s.label}
                        </span>
                    </Link>
                ...
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-20">
                    <h1 className="font-black text-white text-center uppercase leading-none" style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "10vw", mixBlendMode: "difference", letterSpacing: "-0.02em" }}>Something <br /> For Everyone</h1>
                </div>
            </section>
                ...
                <section id="gothic" className="min-h-screen bg-black py-32 px-12 flex flex-col justify-center" style={{ backgroundImage: `url(https://lh3.googleusercontent.com/aida-public/AB6AXuAz0xm-W0iEQ7vWqXrHGwo3DfB5ugIKExvhSg-VdP7pzCulo8MGmzl0H6xUhQLSyfyPY9SIYKr0E-3c9kL714vo6lL9GaEEq6mJznqFNO8CYmjBotQSfyEiYDNkvBHr9__4H7uD5lOV8zzPovI4Iofe8hoCeErk7MY7peTJZFQ-cVo-i1HLEkXbvKdILAn6ReHVy_9hErkJSJm-V5YmmFvkuWISUGKXvHHDONkLQ2RBcgZpmVfUAZB_Uv1cDQ4_-31Ty55G6fwB3TM)`, backgroundBlendMode: "overlay", backgroundSize: "cover" }}>
                    <div className="max-w-[1400px] mx-auto w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            {gothicProducts.map((p, i) => (
                                <RevealSection key={p.id}>
                                    <ProductCard
                                        p={p} bg="#171717" delay={i * 100} slug={`/products/${p.vault_id}`}
                                        idColor="#920703" editionStyle={{ background: "rgba(255,255,255,0.1)", color: "white" }}
                                        nameStyle={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "1.25rem", color: "white", fontWeight: 700 }}
                                        unitStyle={{ color: "#737373" }} borderStyle={{ borderColor: "rgba(255,255,255,0.1)" }} arrowColor="white"
                                    />
                                </RevealSection>
                            ))}
                        </div>
            ...
            {bohemianProducts.length > 0 && (
                <section id="bohemian" className="min-h-screen py-32 px-12 flex flex-col justify-center text-black" style={{ background: "#f5ebe0" }}>
                    <div className="max-w-[1400px] mx-auto w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            {bohemianProducts.map((p, i) => (
                                <RevealSection key={p.id}>
                                    <ProductCard
                                        p={p} bg="white" delay={i * 100} slug={`/products/${p.vault_id}`}
                                        idColor="#8b5e3c" editionStyle={{ background: "rgba(0,0,0,0.05)", color: "black" }}
                                        nameStyle={{ fontFamily: "'Playfair Display', serif", fontSize: "1.5rem", color: "black", fontStyle: "italic", fontWeight: 700 }}
                                        unitStyle={{ color: "rgba(0,0,0,0.5)", fontWeight: 700 }} borderStyle={{ borderColor: "rgba(0,0,0,0.1)" }} arrowColor="black"
                                    />
                                </RevealSection>
                            ))}
                        </div>
            ...
            {avantProducts.length > 0 && (
                <section id="avant" className="min-h-screen bg-white py-32 px-12 flex flex-col justify-center text-black relative overflow-hidden">
                    ...
                    <div className="max-w-[1400px] mx-auto w-full relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            {avantProducts.map((p, i) => (
                                <RevealSection key={p.id}>
                                    <ProductCard
                                        p={p} bg="white" delay={i * 100} slug={`/products/${p.vault_id}`}
                                        idColor="black" editionStyle={{ background: "black", color: "white" }}
                                        nameStyle={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "2.5rem", color: "black", letterSpacing: "-0.02em" }}
                                        unitStyle={{ color: "black", fontWeight: 900 }} borderStyle={{ borderColor: "black" }} arrowColor="black"
                                    />
                                </RevealSection>
                            ))}
                        </div>
            ...
            {streetProducts.length > 0 && (
                <section id="street" className="min-h-screen py-32 px-12 flex flex-col justify-center text-white" style={{ background: "#111", backgroundImage: `url(https://lh3.googleusercontent.com/aida-public/AB6AXuAz0xm-W0iEQ7vWqXrHGwo3DfB5ugIKExvhSg-VdP7pzCulo8MGmzl0H6xUhQLSyfyPY9SIYKr0E-3c9kL714vo6lL9GaEEq6mJznqFNO8CYmjBotQSfyEiYDNkvBHr9__4H7uD5lOV8zzPovI4Iofe8hoCeErk7MY7peTJZFQ-cVo-i1HLEkXbvKdILAn6ReHVy_9hErkJSJm-V5YmmFvkuWISUGKXvHHDONkLQ2RBcgZpmVfUAZB_Uv1cDQ4_-31Ty55G6fwB3TM)`, backgroundSize: "cover", backgroundBlendMode: "overlay" }}>
                    <div className="max-w-[1400px] mx-auto w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            {streetProducts.map((p, i) => (
                                <RevealSection key={p.id}>
                                    <ProductCard
                                        p={p} bg="#262626" delay={i * 100} slug={`/products/${p.vault_id}`}
                                        idColor="#d8ca23" editionStyle={{ border: "1px solid #d8ca23", color: "#d8ca23" }}
                                        nameStyle={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "3rem", color: "white" }}
                                        unitStyle={{ color: "#a3a3a3" }} borderStyle={{ borderColor: "rgba(255,255,255,0.1)" }} arrowColor="#d8ca23"
                                    />
                                </RevealSection>
                            ))}
                        </div>
            ...
            {funkyProducts.length > 0 && (
                <section id="funky" className="min-h-screen py-32 px-12 flex flex-col justify-center text-white relative overflow-hidden" style={{ background: "#0d0d2b" }}>
                    ...
                    <div className="max-w-[1400px] mx-auto w-full relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            {funkyProducts.map((p, i) => (
                                <RevealSection key={p.id}>
                                    <ProductCard
                                        p={p} bg="#1a1a4a" delay={i * 100} slug={`/products/${p.vault_id}`}
                                        idColor="#ffb4a8" editionStyle={{ background: "#ffb4a8", color: "#0d0d2b", fontWeight: 900 }}
                                        nameStyle={{ fontFamily: "'Righteous', sans-serif", fontSize: "1.875rem", color: "white" }}
                                        unitStyle={{ color: "rgba(255,180,168,0.7)", fontWeight: 900 }} borderStyle={{ borderColor: "rgba(255,255,255,0.1)" }} arrowColor="#ffb4a8"
                                    />
                                </RevealSection>
                            ))}
                        </div>
                ...
                <section className="py-48 bg-black px-12 border-t border-white/5">
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {retiredDesigns.map((d: any, i) => (
                                <RevealSection key={d.id}>
                                    <Link href={`/vault`} className="relative aspect-square bg-neutral-900 group grayscale hover:grayscale-0 overflow-hidden block" style={{ transition: "all 1s ease", transitionDelay: `${i * 100}ms` }}>
                                        {d.images?.length > 0 && <img
                                            ...
                                            onMouseLeave={(e) => { (e.currentTarget as HTMLImageElement).style.opacity = "0.2"; }}
                                        />}
                                        <div className="absolute inset-0 flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: "linear-gradient(to top, black, transparent)" }}>
                                            <span className="text-[10px]" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#72d2ff" }}>{new Date(d.created_at).getFullYear()} EDITION</span>
